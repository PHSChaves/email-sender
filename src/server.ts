...
import { VerificationCode, EmailRequest, VerifyCodeRequest, ApiResponse } from './types';
...
function getEmailTemplate(recipientEmail: string, code: string, emailId: string): string {
  const trackingUrl = `http://localhost:${PORT}/api/track/${emailId}`;
...
app.get('/api/track/:emailId', (req: Request, res: Response) => {
  ...
  for (const [email, data] of verificationCodes.entries()) {
    if (data.emailId === emailId && !data.opened) {
      data.opened = true;
      console.log(`📧 Email aberto: ${email}`);
      break;
    }
  }
